...
int camera_init_helper(const char *dev_name, int *fd_ret, int buf_len, int width, int height, int format)
{
	int fd, ret;
...
//DUMMY_FUNC(int, camera_get_frame_helper, NULL, uint8_t *buf, uint32_t *size)
...
//	*out = (uint8_t*)buffers[buf.index].offset;
...
#else
	ret = read(fd, *out, 1225728);
	if (ret == -1) {
		CAMERA_ERR("Failed to read from v4l2 fd : %s\n", strerror(errno));
		return -1;
	}

	CAMERA_DBG("Byte Read :%d\n", ret);
#endif
	return 0;
}
