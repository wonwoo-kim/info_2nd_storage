...
int rknn_init_helper(const char* filename, rknn_context *ctx)
{
	...
	if (model == nullptr) {
		RKNN_ERR("Failed to load model\n");
		return -1;
	}

	ret = rknn_init(ctx, model, model_len, 0);
	if (ret != RKNN_SUCC) {
		RKNN_ERR("Failed to initialize rknn\n");
		return -1;
	}
...
int rknn_run_helper(rknn_context ctx, void *data, uint32_t size, void *out)
{
	...
	if (ret != RKNN_SUCC) {
		RKNN_ERR("Failed to set rknn inputs\n");
		return -1;
	}
