...
INPUT_SIZE_WIDTH = 1064
INPUT_SIZE_HEIGHT = 798
...
if __name__ == '__main__':
    ...
    # Inference
    print('--> Running model')
    outputs = rknn.inference(inputs=[infrared_input])
    print('done')
    ...
    img = np.reshape(img,(INPUT_SIZE_HEIGHT,INPUT_SIZE_WIDTH))
    print(img.shape)
    print(infrared_input.shape)

    cv2.imwrite("out.jpg",img)

    # Evaluate Perf on Simulator
    rknn.eval_perf(inputs=[infrared_input], is_print=True)
